package ups.edu.ec.ArmijosFabian.OperadoraJEE.ups.edu.ec.ArmijosFabian.OperadoraJEE.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/ups.edu.ec.ArmijosFabian.OperadoraJEE.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "ups.edu.ec.ArmijosFabian.OperadoraJEE.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ups.edu.ec.ArmijosFabian.OperadoraJEE.ups.edu.ec.ArmijosFabian.OperadoraJEE.test");
